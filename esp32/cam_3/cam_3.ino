...
void camera_setup(){
  ...
  // Use a lower frame size and slightly lower jpeg quality to reduce bandwidth
  config.frame_size = FRAMESIZE_VGA; // smaller: faster transfer and lower CPU
  config.jpeg_quality = 18; // higher number = lower quality, smaller size
  config.fb_count = 1; // reduce frame buffers to save memory and latency
...
void setup() {
  ...
  const char* mac_check = data["MAC"];
  db.close();
  if (String(mac_check).length()==3)
  {
    GetMACaddress();
    File db1 = SD_MMC.open("/data.json", FILE_WRITE);
    JsonDocument db_write;

    db_write["IP"] = data["IP"];
    db_write["ssid"] = data["ssid"];
    db_write["password"] = data["password"];
    db_write["MAC"] = mac;
  ...
  server.on("/mac", sendMACaddress);
  server.on("/ap_data", handleAPDataExchange);
  server.begin();
...
void sendMACaddress()
{
  server.send(200, "text/plain", String(mac));
  delay(2000);
  server.stop();
